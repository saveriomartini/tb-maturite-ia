...
| 1.7b | **Passe d'attaque de phrase** : 110 énoncés sur 140 commencent par « Nous ». Le parallélisme est un atout *à l'intérieur* d'un domaine — cinq phrases comparables d'un coup d'œil — et un défaut *entre* domaines, visible seulement dans l'export détaillé. Varier le sujet sur la moitié des domaines sans quitter la première personne active : « Nos fournisseurs d'IA figurent dans un inventaire que nous tenons à jour » | S/C | ½j | `src/data/statements.js` | 1.6 | [ ] |
...
| 1.9 | Texte d'attribution (pied de page + export) : Ozkaya et al. 2026 CC BY-NC-ND, Venkatraman 1994, Elia et al. 2024, Bettoni et al. 2021, « outil non officiel » | S | ¼j | `src/data/attribution.js` | — | [x] |
...
| 3.4 | Verdict collant dans `AppHeader`, phases 2–3 : profil atteint, profil suivant, progression | C | ¼j | `AppHeader.vue` | 2.2 | [x] |
